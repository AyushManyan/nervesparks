...
export default function Recommend(){
  ...
  return (
    <div className="flex justify-center items-center min-h-[70vh] bg-white">
      <div className="w-full max-w-xl bg-green-50 rounded-xl shadow-lg p-8 border border-green-200">
        <h2 className="text-2xl font-bold text-green-700 mb-6 text-center">Get Smart Recommendations</h2>
        <form onSubmit={submit} className="flex gap-3 mb-6">
          <input
            value={preferences}
            onChange={e => setPreferences(e.target.value)}
            placeholder="Type your food preferences (e.g., low-calorie)"
            className="flex-1 p-3 border-2 border-green-300 rounded-lg focus:outline-none focus:border-green-500 bg-white"
          />
          <button className="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-2 rounded-lg transition">Search</button>
        </form>
                ...
                <div className="space-y-4">
                  {result.retrieved.map((recipe) => (
                    <div key={recipe._id} className="border p-4 rounded bg-green-50">
                      ...
                      <p><span className="font-semibold">Ingredients:</span> {Array.isArray(recipe.ingredients) ? recipe.ingredients.join(', ') : ''}</p>
                      <p><span className="font-semibold">Instructions:</span> {recipe.instructions}</p>
                      {recipe.nutritional_info && (
                        <p><span className="font-semibold">Nutrition:</span> Calories: {recipe.nutritional_info.calories} kcal, Protein: {recipe.nutritional_info.protein}g{recipe.nutritional_info.carbs !== undefined ? `, Carbs: ${recipe.nutritional_info.carbs}g` : ''}</p>
                      )}
                    </div>
                  ))}
                </div>
